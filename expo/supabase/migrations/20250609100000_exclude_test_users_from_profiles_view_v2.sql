-- テスト用アカウントを一覧ビューから除外（メール・表示名パターン拡張）
...
WHERE NOT (
  lower(coalesce(p.email, '')) = 't@t.com'
  OR p.email ~* '^test-[0-9]+@example\.com$'
  OR p.email ~* '@example\.com$'
  OR p.email ~* '@test\.com$'
  OR lower(coalesce(p.email, '')) LIKE 'test@%'
  OR lower(trim(coalesce(p.name, ''))) = 'test'
);
